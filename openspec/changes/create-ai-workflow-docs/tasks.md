...
- [ ] 5.4 建立 `02-development/shared/testing.md`
  ...
  - 單元測試 / 整合測試 / E2E
  - 測試策略
- [ ] 5.5 建立 `02-development/shared/git-workflow.md` 🎯 **核心**
  - 分支命名規範：`${ENV}_[frontend/backend]_[system]_[collaborator]`（可配置）
  - 環境變數預設值：`${ENV_DEV}=dev`, `${ENV_STAGING}=staging`, `${ENV_PROD}=prod`
  - 功能分支：`feature/`、`fix-issue/`、`hotfix/`
  - 工作流程：`${ENV_DEV}` → `${ENV_STAGING}` → `${ENV_PROD}`
  - 問題回報與修復流程
  - AI Agent 協作時的 Git 操作指引（含首次使用時詢問環境命名偏好）
- [ ] 5.6 建立 `02-development/shared/prompts/` 目錄
- [ ] 5.7 建立 `02-development/shared/prompts/feature-implementation.md`
- [ ] 5.8 建立 `02-development/shared/prompts/bug-fixing.md`
- [ ] 5.9 建立 `02-development/shared/prompts/integration-test.md`
- [ ] 5.10 建立 `02-development/shared/prompts/git-branch-create.md`
  - 根據任務類型建立正確分支的 Prompt
- [ ] 5.11 建立 `02-development/shared/prompts/git-merge-flow.md`
  - 環境間合併流程的 Prompt（使用配置的環境變數）

**驗收標準**：

- validation-framework.md 提供實用的驗證流程
- 測試策略涵蓋各層級
- git-workflow.md 提供完整的分支策略和工作流程
- Git Prompt 範本可直接用於 AI Agent 操作
...
| **核心文件**   | design-system.md、validation-framework.md、git-workflow.md      |
| **案例來源**   | `/Users/ericcai/project/ewill-web/reports/vibe-coding-2026-01/` |
| **版本追蹤**   | CHANGELOG.md + OpenSpec changes                                 |
| **技術棧範例** | Vue3、Python、C# 三種                                           |
| **Git 工作流** | 可配置的三環境分支策略（預設：dev → staging → prod）            |
...
5. **格式一致**：所有文件遵循 config.yaml 定義的格式標準
