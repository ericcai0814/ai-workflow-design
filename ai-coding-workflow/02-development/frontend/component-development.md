...
1. **定義規格**：確定元件名稱、Props、變體
2. **實作元件**：使用設計系統的 tokens
3. **加入測試**：單元測試 + 視覺測試
4. **文件化**：Storybook 或使用範例
...
A: 根據專案結構，通常放在 `src/components/` 或 `src/ui/`。共用元件放在 `shared/`，頁面專屬元件放在對應頁面目錄。

**Q: 何時應該拆分元件？**

A: 當元件超過 200 行、有 5 個以上的 props、或包含多個獨立功能時，考慮拆分。

**Q: 如何處理元件的狀態？**

A: 簡單狀態用 `useState`，複雜狀態考慮 `useReducer`，跨元件狀態參考 `state-management.md`。
